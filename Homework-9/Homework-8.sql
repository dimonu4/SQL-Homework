...
SELECT users.id, users.first_name, users.last_name,users.birthday_at, count(DISTINCT likes.id)AS total_likes  FROM users
LEFT JOIN likes
ON users.id = likes.target_id
AND likes.target_type_id = 2
GROUP BY users.id
...
SELECT SUM(total_likes) AS total_sum_youngest FROM 
(
SELECT users.id, users.first_name, users.last_name,users.birthday_at, count(target_types.id)AS total_likes  FROM users
LEFT JOIN likes
ON users.id = likes.target_id
...
SELECT users.id, users.first_name,users.last_name, count(DISTINCT likes.id) AS total_likes FROM users
LEFT JOIN likes 
ON users.id = likes.user_id
GROUP BY users.id
ORDER BY total_likes
...
count(DISTINCT messages.id)AS activity FROM users
LEFT JOIN likes 
ON users.id = likes.user_id
LEFT JOIN media
ON users.id = media.user_id
